...
{
    /// <summary>
    /// Cmdlet pour exporter une paire de clés Dilithium vers un fichier
    /// </summary>
    [Cmdlet(VerbsData.Export, "DilithiumKeyPair")]
    [OutputType(typeof(string))]
    public class ExportDilithiumKeyPairCommand : PSCmdlet
    {
        [Parameter(
            Mandatory = true,
            Position = 0,
            ValueFromPipeline = true,
            HelpMessage = "Objet DilithiumKeyPairResult contenant les clés à exporter")]
        public DilithiumKeyPairResult KeyPair { get; set; }
        ...
        protected override void ProcessRecord()
        {
            try
            {
                ...
                if (Format.Equals("Text", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    sb.AppendLine("# Dilithium (ML-DSA) Key Pair Export");
                ...
                else
                {
                    // Format Base64/PEM
                    StringBuilder sb = new StringBuilder();
                    sb.AppendLine("-----BEGIN DILITHIUM PUBLIC KEY-----");
                    sb.AppendLine(Convert.ToBase64String(KeyPair.PublicKey));
                    sb.AppendLine("-----END DILITHIUM PUBLIC KEY-----");
                    sb.AppendLine();
                    sb.AppendLine("-----BEGIN DILITHIUM PRIVATE KEY-----");
                    sb.AppendLine(Convert.ToBase64String(KeyPair.PrivateKey));
                    sb.AppendLine("-----END DILITHIUM PRIVATE KEY-----");
                    content = sb.ToString();
                }
                ...
                WriteVerbose($"Paire de clés Dilithium exportée vers: {resolvedPath}");
            }
            catch (Exception ex)
            {
